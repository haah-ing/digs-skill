...
Files live in `digs/`. One file per research thread.
...
2. **Check for existing digs** — search `digs/` for related threads. If one exists, offer to add to it rather than opening a duplicate.
...
- User asks "what am I figuring out about X?" → search `digs/` with expanded keywords, surface active and simmering digs
...
Then move the file to `digs/closed/`. Don't delete — closed digs are useful when a related question reopens.
...
```bash
# Digs on a topic
grep -ril "urban\|city\|walkab\|density\|housing" digs/

# All active digs
grep -rl "Status: active" digs/

# Simmering threads
grep -rl "Status: simmering" digs/
...
If it is not there yet, ask your human if they want to add **Digs: check** to HEARTBEAT.md.
...
- When a book in `pages/` is relevant to an open dig, surface the connection: "You read *Cities for People* in 2025 — your notes might be relevant to the walkability dig."
- When logging a new finding from a book, use `[[author-slug]]` in Sources and optionally add a note to the book file: "Referenced in digs/city-walkability.md — Apr 2026."
...
- Keeping closed digs in the active folder — move them to `digs/closed/` so the signal stays clean
